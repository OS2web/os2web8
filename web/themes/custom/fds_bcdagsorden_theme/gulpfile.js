...
// Reloaders
gulp.task('reload:javascripts', (done) => {
    browserSync.reload();
    done();
});
gulp.task('reload:fonts', (done) => {
    browserSync.reload();
    done();
});
gulp.task('reload:template', (done) => {
    browserSync.reload();
    done();
});

// Builders
gulp.task('build:javascripts', series('clean:javascripts', 'process:javascripts'));
gulp.task('build:styles', series('clean:styles', 'process:styles'));
gulp.task('build:fonts', series('clean:fonts', 'process:fonts'));
gulp.task('build:modernizr', series(parallel('build:javascripts', 'build:styles'), 'clean:modernizr', 'process:modernizr'));

// Define the main build task
gulp.task('build', series(parallel('build:modernizr', 'build:fonts')));

// Watchers
gulp.task('watcher:styles', series('build:styles'));
gulp.task('watcher:javascripts', series('build:javascripts', 'reload:javascripts'));
gulp.task('watcher:fonts', series('build:fonts', 'reload:fonts'));
gulp.task('watcher:templates', series('reload:template'));

// Watch Task
gulp.task('watch', series('build', () => {
    watch('src/styles/**/*.scss', series('watcher:styles'));
    watch('src/javascripts/**/*.js', series('watcher:javascripts'));
    watch('src/fonts/**/*.+(eot|svg|ttf|woff|woff2)', series('watcher:fonts'));
    watch('**/*.+(twig|twig.html|tpl|tpl.php|html)', series('watcher:templates'));
