...
namespace Drupal\fk_d7_migration\Utility;
...
class MigrationHelper {
  ...
  public static $siteUrl = 'https://fredericia.dk';
   ...
   */
   static function findLocalNode($sourceNodeId) {
    $node_migrate_tables = [
      'migrate_map_fk_d7_node_indholdside',
      'migrate_map_fk_d7_node_news',
      'migrate_map_fk_d7_node_borgerdk_indholdside'
    ];
    ...
    // If not present, try to find a node with the similar title
    // Getting connection to migrate database.
    $connection = Database::getConnection('default', 'migrate');
   ...
   *
   * Searches in migrate_map_fk_d7_taxonomy_section.
   *
   * @param $remoteTermId
   *   Remote node ID.
   *
  ...
  static function findLocalSectionTerm($remoteTermId) {
    $term_migrate_tables = [
      'migrate_map_fk_d7_taxonomy_section',
    ];
